...
const ArticleBlogs = () => {
  ...
  const goToBlog = (title) => {
    return () => navigate(`/blog/${title}`); // Return a function to navigate
  };
  ...
  return (
    <div className="pt-20">
      <motion.div
        ...
        transition={{ duration: 0.6, ease: "easeOut" }} // Duration and easing function
      >
      ...
      >
        {article.map((item) => (
          ...
          >
            ...
            />
            <div className="py-5">
              <p className="text-sm text-green-600 font-medium">{item.date}</p>
              <p className="text-lg font-semibold leading-[30px] pt-2">
                {item.title}
              </p>
            </div>
          </motion.div>
